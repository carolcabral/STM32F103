...
#define GPIOx_MASK(_N)            ((GPIO_TypeDef *)(GPIOA_BASE + (GPIOB_BASE-GPIOA_BASE)*((long unsigned int)_N)))
...
public:
    ...
    enum class Error_Type {
		...
		ILLEGAL_COMMAND,		  //Illegal command receive. (it can happen if the MCU take long time to send the cmd)
    };

    SDCard(volatile uint8_t port_cs, uint8_t pin_cs,SPI *dev);
    bool init();
    Type get_type();
...
private:
    ...
    static const uint16_t SD_INIT_TIMEOUT = 3000;
